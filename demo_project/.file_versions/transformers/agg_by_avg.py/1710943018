...
def execute_transformer_action(df: DataFrame, *args, **kwargs) -> DataFrame:
    ...
    action = build_transformer_action(
        ...
        action_code='Quantity > 8',  # Enter filtering condition on rows before aggregation
        arguments=['Unit price'],  # Enter the columns to compute aggregate over
        axis=Axis.COLUMN,
        options={'groupby_columns': ['Branch']},  # Enter columns to group by
        outputs=[
